...
exec(
  `mysql -u ${user} -p${password} -h${host} --ssl=0 -P${port} -e "SHOW DATABASES;"`,
  (error, stdout) => {
    if (error) {
    ...
    const command = `mysql -u ${user} -p${password} -h${host} --ssl=0 -P${port} ${database} -e "DROP DATABASE ${database};"`;
